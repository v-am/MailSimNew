    ...
    class ExecuteSequence
    {
        ...
        private bool MailMove(MailSimOperationsMailMove operation)
        {
            ...
            try
            {
                ...
                // randomly generates the number of emails to move
                if (iterations == 0)
                {
                    random = true;
                    iterations = randomNum.Next(1, mails.Count + 1);
        ...
        public static void FolderMonitorEvent(object Item)
        {
            if (Item == null)
            {
                Log.Out(Log.Severity.Info, Log.eventString, "Unknown event received");
                return;
            }
            ...
            if (Item is IMailItem)
            {
                ...
                Log.Out(Log.Severity.Info, Log.eventString, "New item from {0} with subject \"{1}\"!!", mail.SenderName, mail.Subject);
            }
            else
            {
                Log.Out(Log.Severity.Info, Log.eventString, "Event received but with unknown type " + Item.GetType().ToString());
            }
        }
