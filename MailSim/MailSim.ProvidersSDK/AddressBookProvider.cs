...
namespace MailSim.ProvidersSDK
{
    class AddressBookProvider : IAddressBook
    {
        ...
        {
            var pagedUsers = _adClient.Users.ExecuteAsync().Result;
            ...
            {
//                Util.Synchronize(async () => pagedUsers = await pagedUsers.GetNextPageAsync());
...
//                .Where(g => g.DisplayName.Equals(dLName, StringComparison.OrdinalIgnoreCase))
